...
def detect_and_handle_outliers(
    ...
    for col in columns:
        ...
        # Apply selected human-guided mitigation strategy
        if action == "cap":
            # Cap extreme anomalies at their respective statistical thresholds
            df_out[col] = df_out[col].clip(lower=lower_bound, upper=upper_bound)
        elif action == "nullify":
            # Set outliers as NaN to let imputation step treat them as missing inputs
            df_out.loc[outliers_mask, col] = np.nan
        elif action == "remove":
            # Filter rows containing outlier cells in this column
...
def impute_missing_values(
    ...
    for col in columns:
        ...
        else: # Time-series linear interpolation (industry standard for continuous sensors)
...
def run_engineering_pipeline(
    df_raw: pd.DataFrame,
    df_ingested: pd.DataFrame, # Ingested/Type-Coerced baseline to compare distribution shifts mapping
    ...
    """
    Executes Module 2 (Engineering & Sanitization). Runs outlier mitigation, 
    time interpolation, scaling, and generates statistical drift alerts.
    """
    # 1. Capture original baseline state (using df_ingested)
    df_baseline = df_ingested.copy()
    
    # 2. Outlier Detection and Human Guidance strategy
    ...
    # Calculate Completeness Metric BEFORE scaling transforms standard values
    ...
    # 5. Measure and flag statistical shifts between baseline (original values) and imputed/treated dataset
    # We pass the pre-scaled imputed representation to trace density shifts, avoiding comparing raw to standardized scales 
    # which always trigger 100% drift due to range changes.
    ...
    # Calculate Sanitization Health Score
    health_results = calculate_engineering_health(
        null_percentage=initial_null_impact, # Penalize based on raw incoming data missing impact
